...
# Input a pair of sequencing FastQ files gunzip, read, write out smaller broken down files in a format suitable for 
# submitting job array, and re-gzip each one in turn Breakdown by lane & cellID Keep order. This should work equally 
# well for single-end reads and can take any number of files as arguments. TESTED
if (@ARGV != 7) {
...
my $infile1 = $ARGV[0]; my $infile2 = $ARGV[1]; my $barcode_pos = $ARGV[2]; my $barcode_len = $ARGV[3]; my 
$barcode_index_file = $ARGV[4]; my $MAXmismatch = $ARGV[5]; my $OUTprefix = $ARGV[6]; if ($OUTprefix =~ 
/^(.+)\/[^\/]$/) {
	if ($1 ne ".") {
		system("mkdir -p $1");
	}
}
my %CellBarcodes = (); my %ofhs1 = (); my %ofhs2 = (); open(my $ifh, $barcode_index_file) or die "Cannot open 
$barcode_index_file\n"; my $index=1; while (<$ifh>) {
	chomp;
...
my $NotProperBarcodes = 0; my $NotPossibleCell = 0; my $AmbiguousCell = 0; my $ExactMatch = 0; my $Mismatch = 0; my 
$total_reads = 0; my $OutputReads = 0; open(my $ifh1, $infile1) or die $!; open(my $ifh2, $infile2) or die $!; 
while(<$ifh1>) {
	my $file1line=$_;
	my $file2line = <$ifh2>;
	if ($file1line =~ /^@/) {
...
print STDERR " Doesn't match any cell: $NotPossibleCell Ambiguous: $AmbiguousCell Exact Matches: $ExactMatch Contain 
Mismatches: $Mismatch Input Reads: $total_reads Output Reads: $OutputReads\n"; close($ifh1); close($ifh2); foreach my 
$ofh1 (keys(%ofhs1)) {close($ofhs1{$ofh1});} foreach my $ofh2 (keys(%ofhs2)) {close($ofhs2{$ofh2});} sub my_max {
