...
#
#Author:
# Chris Williams Harvard Informatics And Scientific Applications http://informatics.fas.harvard.edu Demultiplex an 
# Illumina fastq file
use warnings FATAL => "all"; use strict; use POSIX; use Getopt::Long; use Data::Dumper; use List::MoreUtils qw(uniq); 
use constant { true => 1, false => 0 }; my $fastq_file = ""; my $barcodes_file = ""; my $outdir = "."; my 
$max_mismatches = 0; my $filter_n = 0; my $crop_n = 0; my $ignore_barcode_n = 0; my $help = 0; 
...
foreach my $barcode (keys %bh) {
  open( my $fh, ">", $outdir . "/" . $bh{$barcode}{"sample"} . "_" . $barcode . ".fastq") or die "Error: Coudln't open 
file for writing: $!";
  $bh{$barcode}{"filehandle"} = $fh;
}
open(FA, "<$fastq_file") or die "Can't open file $fastq_file\n"; my $i = 0; my ($name, $read, $plus, $qual) = 0; 
while( my $line = <FA> ) {
  $i++;
  chomp($line);
  if ($i % 4 == 1) {
...
$qual\n");
    ...
    if ($filter_n && $read =~ m/[nN]/) { #skip read if filter-n option was specified, and read contains an uncalled 
base
      next;
    }
    my $num_hits = 0;
...
exit 0; sub hd { #calculate hamming distance
  my ($k,$l) = @_;
  my $len = length($k);
  my $num_mismatch = 0;
  for (my $i=0; $i<$len; $i++) {
